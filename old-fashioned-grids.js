...
class Cell extends Component {
  constructor(value, type = CellType.DATA, ...classes) {
    super([type, "grid-cell", type].concat(...classes));
...
class SortableColumn extends ColumnDecorator {
  ...
  getHeader() {
    return new Container(new NoLayout(), "grid-cell").add(
      new Container()
        .add(this.child.getHeader(), Position.WEST)
        .add(this.sortButton, Position.CENTER)
    );
  }
  getCell(dataRow) {
...
class Pagination extends Container {
  constructor(resultsPerPage = 1, currentPage = 1) {
    ...
    this.backButton = new Button("Go Back").addActionListener((e) =>
    ...
    this.nextButton = new Button("Go to Next Page").addActionListener((e) =>
      this.changePage("next")
    );
    this.add(this.backButton, Position.WEST)
      .add(this.label, Position.CENTER)
  ...
  onUpdate(newValue) {
    ...
    for (let dataRow of newValue.content) {
      const rowElement = new Row(
        this.rowClickListeners.length > 0
          ? (e) =>
              this.rowClickListeners.forEach((listener) => listener(dataRow))
          : null
      );
...
class TableSearch extends TableDecorator {
  constructor(tableContainer) {
    ...
    const searchButton = new Button("Search").addActionListener((e) =>
      this.onChange()
    );
    this.add(
      new Container()
        .add(this.searchField, Position.CENTER)
        .add(searchButton, Position.EAST),
      Position.NORTH
    );
...
class TablePagination extends TableDecorator {
  constructor(tableContainer, resultsPerPage, currentPage) {
    ...
    this.add(this.pagination, Position.SOUTH);
    this.add(this.child, Position.CENTER);
  }
  getValue() {
    const params = this.child.getValue();
...
class TableChangeManager {
  constructor(changeListener = (e) => {}) {
    ...
    this.rules = [
      new TableChangeRule("searchTerm", (tableResponse) => {
  ...
  onChange(e) {
    ...
    if (this.hasEverChanged) {
      this.rules
        .filter((rule) => changedKeys.includes(rule.tableParamsKey))
        .forEach((rule) => {
    ...
    if (!this.value) {
      ...
      for (let keyOfNewValue of keysOfNew) {
        if (!keysOfOld.includes(keyOfNewValue)) {
          ...
          JSON.stringify(this.value[keyOfNewValue]) !==
          JSON.stringify(e[keyOfNewValue])
        ) {
          changedKeys.push(keyOfNewValue);
        }
      }
...
class DecoratedTableBuilder {
  ...
  build() {
    return this.table.addChangeListener((e) =>
      this.tableChangeManager.onChange(e)
    );
  }
}
